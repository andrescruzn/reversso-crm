...
<div class="max-w-7xl mx-auto px-2 md:px-0">
    ...
    <form action="{{ route('logistics.index') }}" method="GET" class="flex flex-col md:flex-row items-stretch md:items-center gap-3 mb-8">
        ...
        <button type="submit" class="bg-gray-900 text-white px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest">Filtrar</button>
        <a href="{{ route('logistics.index') }}" class="bg-gray-100 text-gray-500 px-6 py-3.5 rounded-2xl text-[10px] font-black uppercase flex items-center justify-center tracking-widest">Limpiar</a>
    </form>
        ...
        <table class="min-w-full divide-y divide-gray-100">
            <tbody>
                ...
                <tr class="hover:bg-orange-50/30">
                    <td class="px-8 py-5 text-xs font-black text-gray-900 uppercase leading-none">
                        {{ $tracking->user->name }}
                        <span class="block text-[8px] text-reversso mt-1 italic font-bold">Destino: {{ $tracking->destination }}</span>
                    </td>
                    <td class="px-8 py-5 text-center">
                        <span class="bg-green-100 px-3 py-1 rounded-full text-[9px] font-black text-green-700 uppercase">En Ruta</span>
                    </td>
                    <td class="px-8 py-5 text-right">
                        <a href="{{ route('logistics.trip.show', $tracking->id) }}" class="text-[9px] font-black text-reversso border border-orange-200 px-4 py-2 rounded-xl uppercase">Detalles</a>
                    </td>
                </tr>
                @endforeach
                @foreach($onlyAttendance as $waiting)
                <tr class="bg-gray-50/30">
                    <td class="px-8 py-5 text-xs font-black text-gray-400 uppercase leading-none">
                        {{ $waiting->user_name }}
                        <span class="block text-[8px] mt-1 text-gray-400 italic font-bold">Presente en Planta</span>
                    </td>
                    <td class="px-8 py-5 text-center">
                        <span class="bg-gray-100 px-3 py-1 rounded-full text-[9px] font-black text-gray-400 uppercase">Esperando</span>
                    </td>
                    <td class="px-8 py-5 text-right text-[8px] font-black text-gray-300 italic uppercase">Sin Viaje</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-8 py-5 text-right">
                            <div class="flex justify-end items-center gap-3">
                                <a href="{{ route('logistics.trip.show', $trip->id) }}" class="text-[9px] font-black text-gray-900 border-b-2 border-reversso uppercase italic">Detalle</a>

                                @if($trip->approved_at)
                                {{-- BOTÓN DESAPROBAR (X ROJA) --}}
                                <form action="{{ route('logistics.disapprove', $trip->id) }}" method="POST" onsubmit="return confirm('¿Revertir aprobación?')">
                                    @csrf
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-8 py-10 text-center text-[10px] font-black uppercase text-gray-300 italic">Sin registros para el periodo seleccionado</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
